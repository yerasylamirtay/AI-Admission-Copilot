...
export default function Step1Landing({ onStart, onLoginSuccess }: Step1LandingProps) {
  ...
  async function handleEmailAuth(event: React.FormEvent) {
    ...
    try {
      ...
      if (raw.toLowerCase().includes('email not confirmed')) {
        ...
        setErrorMsg(raw || 'Не удалось войти. Проверь email и пароль.');
      }
    }
  ...
  async function handleGoogleAuth() {
    ...
    try {
      ...
      setErrorMsg(error.message || 'Не удалось войти через Google.');
    }
  }
...
<button type="button" onClick={handleGoogleAuth} className="btn-secondary flex w-full items-center justify-center gap-2 py-3 text-sm mb-4">
