...
export async function POST(request: Request) {
  try {
    ...
    if (!checkAndIncrementBudget().allowed) return NextResponse.json({ explanation: fallback, source: 'fallback' });
    try {
      const explanation = await callClaude(systemPrompt, userMessage, 200);
